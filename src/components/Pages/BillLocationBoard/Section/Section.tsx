...
const getSectionBillCount = (billsInStatuses: any) => {
  ...
  if (billsInStatuses) {
    Object.keys(billsInStatuses).forEach((key) => {
      Object.keys(billsInStatuses[key]).forEach(
        (sublocation) =>
          (totalCount += billsInStatuses[key][sublocation].length)
      );
    });
    return totalCount;
  }
...
const Section = ({ sectionData, groupTitle, sectionTitle }: SectionProps) => {
  ...
  const measuresInKanbanLocations = useMeasureStore
    .getState()
    .getMeasuresSortedIntoKanbanLocations();
  ...
  const ErrorSection = () =>
    Object.keys(billsInStatuses).map((statusKey) => (
      <div
        className="status-container"
        key={`${groupTitle}-${section}-${statusKey}`}
      >
        <StatusTitle title={statusKey} key={statusKey} />
        {billsInStatuses[statusKey]['uncategorized'].map((bill: any) => (
          <Bill billId={bill.id} key={bill.id} />
        ))}
      </div>
    ));
    ...
    <div className="section" key={`${groupTitle}-${section}`}>
      ...
      {data &&
        data.map((status: { data: any; status: string }) => (
          <div
            className="status-container"
            key={`${groupTitle}-${section}-${status.status}`}
          >
            <StatusTitle title={status.status} key={status.status} />
            <div
              className="sublocations-container"
              key={'sublocation' + status.status}
            >
              {status.data.map((sublocation: string) => (
                <React.Fragment key={sublocation}>
                  {sublocation && (
                    <div className="sublocation-title" key={sublocation}>
                      {sublocation}
                    </div>
                  )}
                  {(billsInStatuses?.[status.status]?.[sublocation] || []).map(
                    (bill: any) => (
